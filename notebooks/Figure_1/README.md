...
Reproduces panels B, F, G, H, I, J from Figure 1
...
Open and run `01-UMAP_plot_Figure_1b_6M.ipynb`. Point `DATA_PATH` at `filtered_lognorm.zarr`. Generates the 6M-cell UMAP coloured by cell type.
...
### 5. Perturbation distribution — panels F, G, I, J

Two R markdown notebooks cover the remaining panels:

```bash
Rscript -e "rmarkdown::render('Fig1_GFJ.rmd')"
Rscript -e "rmarkdown::render('04-perturbaion_distribution_Figure_1_GIJ_fixed.rmd')"
```
