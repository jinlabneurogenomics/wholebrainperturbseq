...
# ============================================================
# Base: python:3.11 (Debian bookworm).
...
RUN apt-get update -y && apt-get install -y --no-install-recommends \
    ...
    gnupg \
    wget \
    && wget -qO- https://cloud.r-project.org/bin/linux/debian/marutter_pubkey.asc | tee /etc/apt/trusted.gpg.d/cran_debian_key.asc \
    && echo "deb [arch=amd64] https://cloud.r-project.org/bin/linux/debian bookworm-cran40/" > /etc/apt/sources.list.d/cran_r.list \
    && apt-get update -y \
